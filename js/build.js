...
// Initials derived from a name, e.g. "Manos Tsakiris" -> "MT".
...
// Render a member's portrait: the photo if its file exists on disk, otherwise
// a tasteful initials avatar so missing/empty images don't show a broken icon.
function memberPortrait(m) {
  if (m.photo && fs.existsSync(path.join(ROOT, m.photo))) {
    return `<img src="${m.photo}" alt="${m.name}">`;
  }
  return `<div class="avatar-placeholder" aria-label="${m.name}">${initialsFromName(m.name)}</div>`;
}
